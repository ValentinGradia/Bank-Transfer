...
namespace Bank.Balance.WebAPI.External.ServiceBusReceive;
...
//When the app starts, it calls ExecuteAsync() (keeps alive until the app is down) method to start the background task. When the app is shutting down, it calls StopAsync() method to stop the background task.
{
    ...
    public ServiceBusReceiveService(IConfiguration configuration, IMediator mediator)
    {
        ...
        {
            ReceiveFromTopicConstants.BALANCE_INITIATED,
            ReceiveFromTopicConstants.TRANSFER_CONFIRMED_BALANCE,
            ReceiveFromTopicConstants.TRANSFER_FAILED_BALANCE
        };
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        // All processors start listening simultaneously. Each one watches its own subscription and fires Process() when a           
